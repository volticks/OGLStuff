...
Projectile::Projectile(uint32_t vaoID, uint32_t vboID, float* startingVert, uint32_t vertLen, Shader& prog, bool collidable, std::vector<VertAttribute> vas, ShapeCollisionFuncT cFunc, bool isTextured, uint32_t texID, float vel, glm::vec3 direction, glm::vec3 startingPos, bool gravEnabled) :
...
void ProjectileContainer::drawProjectiles(ObjectContainer& colObjects) {
	...
	for (auto& pd : projectileData) {
		...
		//if (projTemplate.gravEnabled)
		//	pd->position.y -= Util::gravPower;
